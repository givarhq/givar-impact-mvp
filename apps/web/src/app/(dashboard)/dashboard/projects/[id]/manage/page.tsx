        ...
        <div className="w-full min-w-0 space-y-4 md:space-y-6 animate-in fade-in duration-500 pb-24">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 min-w-0">
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <Badge variant="secondary" className="bg-primary/5 text-primary border-primary/10 font-bold text-[10px] rounded-3xl px-2.5">
                                Management Console
                            </Badge>
                            {isFullyCompleted && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start min-w-0">
                ...
                <div className="lg:col-span-8 space-y-6 min-w-0">
                    ...
                    {finalReport && (
                        <Card className="rounded-3xl border-emerald-500/20 bg-emerald-500/[0.03] shadow-sm overflow-hidden animate-in slide-in-from-top-2">
                            <CardHeader className="bg-emerald-500/10 border-b border-emerald-500/10 py-4 px-6 flex flex-row items-center gap-3">
                                ...
                                <CardTitle className="text-sm font-bold text-emerald-800">Final impact report</CardTitle>
                            </CardHeader>
                            <CardContent className="p-6 md:p-8 space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                    <div className="space-y-3">
                                        <h4 className="text-lg font-bold text-emerald-950 leading-tight">{finalReport.title}</h4>
                                        <p className="text-sm text-emerald-900/80 font-medium leading-relaxed">
                                            {finalReport.content}
                                        </p>
                                    </div>
                                    {finalReport.imageUrl && (
                                        <div className="relative aspect-video rounded-2xl overflow-hidden border border-emerald-500/20 shadow-md">
                                            <Image src={finalReport.imageUrl} alt="Impact" fill sizes="400px" className="object-cover" />
                                        </div>
                                    )}
                                </div>
                            </CardContent>
                    ...
                    {otherUpdates.length > 0 && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 px-1">
                                ...
                                <h4 className="text-sm font-bold text-foreground">Project updates</h4>
                            </div>
                            <div className="grid gap-3">
                                {otherUpdates.map((update) => (
                                    <Card key={update.id} className="rounded-3xl border-border/40 bg-card p-5 shadow-sm">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="space-y-1">
                                                <Badge variant="secondary" className="text-[9px] font-bold rounded-3xl bg-muted border-none mb-1">
                                                    {update.type.replace(/_/g, ' ')}
                                                </Badge>
                                                <h5 className="text-sm font-bold text-foreground">{update.title}</h5>
                                                <p className="text-xs text-muted-foreground font-medium leading-relaxed">{update.content}</p>
                                            </div>
                                            <span className="text-[10px] font-bold text-muted-foreground whitespace-nowrap">{formatDate(update.createdAt).split(',')[0]}</span>
                                        </div>
                                    </Card>
                                ))}
                            </div>
