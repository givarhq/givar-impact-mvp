...
export const TransparencyCard = memo(function TransparencyCard({ project }: TransparencyCardProps) {
    ...
    const spc = currencySymbol.length > 1 ? ' ' : '';
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl md:text-3xl font-bold text-primary tracking-tight whitespace-pre">
                                {currencySymbol}{spc}{formatNoDecimals(totalRaised)}
                            </span>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2">
                        <div className="p-3 rounded-2xl border border-border/60 flex flex-col items-center justify-center text-center space-y-1.5 shadow-sm bg-muted/10">
                            ...
                            <div className="font-bold text-sm text-foreground whitespace-pre">{currencySymbol}{spc}{formatNoDecimals(totalTarget)}</div>
                        </div>
                        <div className="p-3 rounded-2xl border border-border/60 flex flex-col items-center justify-center text-center space-y-1.5 shadow-sm bg-muted/10">
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                                <AlertCircle className="h-3.5 w-3.5 text-amber-500" /> Remaining
                            </div>
                            <div className="font-bold text-sm text-foreground whitespace-pre">{currencySymbol}{spc}{formatNoDecimals(totalRemaining)}</div>
                        </div>
                    </div>
                </div>
            </Card>
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-end text-[10px] sm:text-[11px] font-bold">
                                    <span className="text-foreground truncate whitespace-pre">
                                        {currencySymbol}{spc}{formatNoDecimals(raisedInCurrentPhase)} raised <span className="text-muted-foreground font-medium mx-0.5">of</span> {currencySymbol}{spc}{formatNoDecimals(currentPhaseTargetMinor)}
                                    </span>
