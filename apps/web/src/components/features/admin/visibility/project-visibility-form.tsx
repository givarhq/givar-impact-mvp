...
export function ProjectVisibilityForm({ project, globalConfig }: ProjectVisibilityFormProps) {
    ...
    const handleUpdate = async () => {
        setIsSaving(true);
        const toastId = toast.loading("Updating discovery metadata...");
        try {
            await ApiService.admin.updateProjectWeights(project.id, data);
            toast.success("Project ranking node updated", { id: toastId });
        } catch (e) {
            toast.error("Failed to commit weights", { id: toastId });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-4xl">
            <Card className="rounded-[32px] border-border/40 bg-card shadow-sm overflow-hidden border-2">
                ...
                <CardContent className="p-6 md:p-8 space-y-10">
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            <SelectTrigger className="h-12 rounded-2xl bg-muted/20 border-border/40 font-bold text-sm">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="rounded-2xl shadow-xl">
                                <SelectItem value="APPROVED" className="font-bold text-xs">Approved (Default Discovery)</SelectItem>
                                <SelectItem value="FLAGGED" className="font-bold text-xs text-amber-600">Flagged (Restricted Discovery)</SelectItem>
                                <SelectItem value="HIDDEN" className="font-bold text-xs text-destructive">Hidden (Blocked from Feed)</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
