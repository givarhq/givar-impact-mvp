...
export function ProposalReview({ proposal }: ProposalReviewProps) {
  ...
  const handleApprove = async () => {
    if (!confirm('Promote to Live Project? This cannot be undone.')) return;
    setIsProcessing(true);
    try {
  ...
  return (
    <div className="space-y-6 pb-20">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            {/* RIGHT: Trust & Controls (3 cols - Sticky) */}
                        ...
                        <CardContent className="p-5 space-y-3">
                            ...
                            <Dialog>
                                <DialogTrigger asChild>
                                    <Button 
                                        variant="ghost" 
                                        className="w-full h-9 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive rounded-xl"
                                        onClick={() => setActionType('reject')}
                                    >
                                        <X className="mr-2 h-3.5 w-3.5" /> Reject Proposal
                                    </Button>
                                </DialogTrigger>
                                <DialogContent>
