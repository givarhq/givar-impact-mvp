...
export function CorporateSponsorshipTrigger({ projectId, projectCurrency }: { projectId: string; projectCurrency: string }) {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            ...
            img1.onload = () => {
                setResolvedLogoUrl(clearbitUrl);
    ...
    const handleDonorAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (rawDonor > 0 && fxRates[donorCurrency]) {
            const rate = 1 / fxRates[donorCurrency];
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (logoMode === 'auto' && resolvedLogoUrl) {
                finalLogoUrl = resolvedLogoUrl;
            } else if (logoMode === 'upload' && uploadedLogo) {
                finalLogoUrl = uploadedLogo.key;
            }
                            ...
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <Tabs value={logoMode} onValueChange={(v: any) => setLogoMode(v)} className="w-[180px]">
                                    <TabsList className="h-8 bg-muted/50 p-0.5 rounded-2xl w-full border border-border/40">
                                        <TabsTrigger value="auto" className="rounded-[14px] text-[10px] font-bold gap-1.5 h-full data-[state=active]:bg-background data-[state=active]:shadow-sm flex-1"><Globe className="h-3 w-3" /> URL</TabsTrigger>
                                        ...
                                        {uploadedLogo ? (
                                            <div className="flex items-center justify-between p-2 bg-background border border-border/60 rounded-xl shadow-sm">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-xs font-bold text-emerald-600 flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> Custom logo uploaded</span>
                                                </div>
