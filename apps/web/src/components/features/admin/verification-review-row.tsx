...
import { Check, X, FileText, Loader2, CheckCircle2 } from 'lucide-react';
...
export function VerificationReviewRow({ profile }: { profile: any }) {
  ...
  if (isProcessed) {
    return (
      <tr className="bg-emerald-500/5 transition-all duration-500 opacity-60">
        <td colSpan={3} className="px-6 py-4 text-xs font-bold text-emerald-600 uppercase tracking-widest italic">
          Processed & Synced to Ledger
        </td>
        ...
        <div className="flex justify-end gap-2">
          <Dialog>
            <DialogTrigger asChild>
              <Button size="icon" variant="ghost" className="h-9 w-9 text-destructive hover:bg-destructive/10 rounded-xl" disabled={isLoading}>
                <X className="h-4 w-4" />
              </Button>
            </DialogTrigger>
