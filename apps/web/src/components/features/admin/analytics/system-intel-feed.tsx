...
import { Zap, ShieldCheck, AlertCircle, ArrowRight, ClipboardList, Flag } from 'lucide-react';
...
export const SystemIntelFeed = memo(function SystemIntelFeed({ report }: { report: any }) {
    ...
    const insights: IntelItem[] = [
        {
            id: 'reports',
            type: report.summary.pendingReportsCount > 0 ? 'RISK' : 'SUCCESS',
            title: 'Community Reports',
            desc: report.summary.pendingReportsCount > 0
                ? `${report.summary.pendingReportsCount} causes have been flagged for review.`
                : 'No active community reports.',
            target: '/admin/projects?status=SUSPENDED',
            icon: Flag
        },
        {
            id: '1',
        ...
        <Card className="rounded-3xl border-border/40 bg-card shadow-sm overflow-hidden flex flex-col h-[420px]">
            ...
            <CardContent className="p-0 flex-1 overflow-y-auto no-scrollbar">
                <div className="divide-y divide-border/40">
                    {insights.map((item) => {
                        const Icon = item.icon || (item.type === 'RISK' ? AlertCircle : item.type === 'ACTION' ? ClipboardList : ShieldCheck);
                        return (
                            <div
                                key={item.id}
