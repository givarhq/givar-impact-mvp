...
export function UserForensicView({ user }: UserForensicViewProps) {
    ...
    const handleImpersonate = async () => {
        if (!confirm(`Launch Forensic Proxy for ${user.email}? You will view the platform exactly as they do. All actions are logged.`)) return;
        ...
        try {
            ...
            // Hard navigation to purge router cache and force banner mount
    ...
    const handleStatusToggle = async () => {
        ...
        const confirmMsg = action === 'LOCK'
            ? `Restrict all access for ${user.email}? This will terminate their active sessions.`
            : `Restore global access for ${user.email}?`;

        if (!confirm(confirmMsg)) return;

        setIsProcessing(true);
        try {
            await ApiService.admin.updateUserStatus(user.id, action as 'LOCK' | 'UNLOCK');
